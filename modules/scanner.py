...
class Scanner:
    ...
    def nuclei(self, template, hosts, output):
        ...
        os.system("nuclei -stats -retries 1 -t {3}nuclei/{0}{4} -timeout 6 -l {1} -o {2}".format(template, hosts, output, self.templates, severity))
        return
    ...
    def jaeles(self, template, hosts, output):
        ...
        if len(self.severity) > 0:
            severity = self.severity
        os.system("jaeles scan --retry 1 --no-background -c 80 --rootDir {3}jaeles/ -s {3}jaeles/{0}/{5} --timeout 6 -U {1} -O {2} -o {4}/jaeles --no-db --chunk true ".format(
    ...
    def vizscan(self):
        ...
        if(os.path.exists(out)):
            os.system("rm {0}".format(out))
        os.system("mkdir {0}/vizscan".format(path))
